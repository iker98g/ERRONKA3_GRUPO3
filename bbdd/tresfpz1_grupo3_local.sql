...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-11-2019 a las 10:50:59
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `spAllHabitaciones` ()  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spBorrarReserva` (IN `pId` INT)  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spComprobarDisponibilidad` (IN `pFechaInicio` DATE, IN `pFechaFin` DATE)  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spCrearUsuario` (IN `pNombre` VARCHAR(50), IN `pApellido` VARCHAR(50), IN `pUsuario` VARCHAR(50), IN `pContrasena` VARCHAR(50), IN `pAdmin` TINYINT(1))  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spInsertReserva` (IN `pIdHabitacion` INT, IN `pIdUsuario` INT, IN `pFechaInicio` DATE, IN `pFechaFin` DATE, IN `pPrecioTotal` DECIMAL(10,2))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spModificarHabitacion` (IN `pId` INT, IN `pTipo` VARCHAR(50), IN `pImagen` VARCHAR(100), IN `pPrecio` DECIMAL(10,2))  NO SQL
UPDATE habitaciones
SET habitaciones.tipo = pTipo, habitaciones.imagen = pImagen, habitaciones.precio = pPrecio
WHERE habitaciones.idHabitacion = pId$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spModificarReserva` (IN `pId` INT, IN `pIdHabitacion` INT, IN `pIdUsuario` INT, IN `pFechaInicio` DATE, IN `pFechaFin` DATE, IN `pPrecioTotal` DECIMAL(10,2))  NO SQL
UPDATE reservas
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spModificarUsuario` (IN `pId` INT, IN `pNombre` VARCHAR(50), IN `pApellido` VARCHAR(50), IN `pUsuario` VARCHAR(50), IN `pAdmin` TINYINT(1))  NO SQL
UPDATE usuarios
SET usuarios.nombre = pNombre, usuarios.apellido = pApellido, usuarios.usuario = pUsuario, usuarios.admin = pAdmin
WHERE usuarios.idUsuario = pId$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spReservasPorUsuario` (IN `pIdUsuario` INT(11))  NO SQL
SELECT * FROM reservas WHERE reservas.idUsuario = pIdUsuario$$
...
--
-- Volcado de datos para la tabla `historicoreservas`
--

INSERT INTO `historicoreservas` (`historicoIdReserva`, `historicoIdHabitacion`, `historicoIdUsuario`, `historicoFechaInicio`, `historicoFechaFin`, `historicoPrecioTotal`) VALUES
(42, 4, 10, '2019-11-03', '2019-11-04', '95.00');
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `spHistoricoReservas` ON SCHEDULE EVERY 1 DAY STARTS '2019-11-05 23:59:59' ENDS '2019-11-06 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM `reservas` WHERE reservas.fechaFin < NOW()$$
